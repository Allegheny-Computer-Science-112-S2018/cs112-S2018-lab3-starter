...
if [ "$CHECK" = true ]; then
  ...
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/labthree/ \
                                     --checkfiles Reverser.java --multicomments 4 --language Java
  determine_exit_code $?
  # --> GatorGrader CHECK: at least 9 "println()" fragment exists in the code
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/labthree \
                                     --checkfiles Reverser.java --fragments "println(" --fragmentcounts 12
  determine_exit_code $?
  # --> GatorGrader CHECK: at least 3 "@Test" fragment exists in the code
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/test/java/labthree \
                                     --checkfiles TestReverser.java --fragments "@Test" --fragmentcounts 6
  determine_exit_code $?
  # --> GatorGrader CHECK: running the program produces exactly 20 lines of output
  python3 gatorgrader/gatorgrader.py --nowelcome --commands "gradle -q --console plain run" --outputlines 28
  determine_exit_code $?
